...
import { ChatPicker, CHAT_ACCENT } from "@/components/ChatPicker";
...
function LinkifiedText({ text, isMe }: { text: string; isMe: boolean }) {
  ...
  const linkColor = isMe ? "#FFE9A8" : "#2F6FED";
...
function ExpandableMessageText({ text, isMe }: { text: string; isMe: boolean }) {
  ...
  const toggleColor = isMe ? "rgba(255,255,255,0.85)" : "#2F6FED";
...
const COMPOSER_HIGHLIGHT_COLORS: Record<string, string> = {
  ...
  phone: "#6BA547", mention: "#2F6FED", hashtag: "#2F6FED",
};
...
function AvatarRing({ name, color, size = 46, src }: { name: string; color?: string; size?: number; src?: string | null }) {
  const { t } = useTranslation();
  return (
    <div style={{ borderRadius: "50%", padding: 2, background: "linear-gradient(135deg,#2F6FED 0%,#2F6FED 50%,#FFC93C 100%)", flexShrink: 0 }}>
        ...
        <div className="relative w-full max-w-sm rounded-3xl shadow-2xl z-10 flex flex-col"
          ...
          <div className="px-5 py-4 space-y-4 overflow-y-auto">
            ...
            <button
              ...
              style={{ background: "#2F6FED", opacity: sendingRequest ? 0.7 : 1 }}>
        ...
        <div className="px-5 py-4 space-y-4 overflow-y-auto">
          ...
          {searching && (
            <div className="flex justify-center py-4">
              <Loader className="h-4 w-4 animate-spin" style={{ color: "#2F6FED" }} />
            </div>
          )}
      ...
      <div className="relative w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b shrink-0" style={{ borderColor: "var(--border,#f0f0f0)" }}>
          <div className="flex items-center gap-2">
            <Bell className="h-5 w-5" style={{ color: "#2F6FED" }} />
        ...
        <div className="overflow-y-auto flex-1 px-5 py-4 space-y-3">
          {loading && <div className="flex justify-center py-8"><Loader className="h-5 w-5 animate-spin" style={{ color: "#2F6FED" }} /></div>}
...
// Constantes e picker de emoji/gif/sticker vivem agora em ChatPicker.tsx,
// partilhados entre este ChatPanel (DMs) e o SalaPanel (salas).
...
const STUDIO_CARDS: StudioCard[] = [
  ...
  { id: "invite",       label: "Convite",       emoji: "🎟️", bg: "linear-gradient(135deg,#1F2937 0%,#2F6FED 100%)", defaultTextColor: "#F5D67A", accent: "✨" },
...
function AudioMsg({ url, isMe, knownDur }: { url: string; isMe: boolean; knownDur?: number }) {
  ...
  const playBg      = isMe ? "rgba(255,255,255,0.2)" : "#2F6FED";
  const barFilled   = isMe ? "#2F6FED" : "#2F6FED";
    ...
    <div className="fixed inset-0 z-[100] flex flex-col" style={{ background: "var(--s0)" }}>
      ...
      <div className="px-4 py-4 border-t shrink-0" style={{ borderColor: "var(--border-subtle)" }}>
        <button disabled={!canSend}
          ...
          style={{ background: "linear-gradient(135deg,#8B5CF6,#2F6FED)" }}>
          ✨ Enviar Mensagem
        </button>
      </div>
    </div>,
        ...
        <div ref={thumbStripRef} className="flex items-center gap-2 px-4 py-3 overflow-x-auto shrink-0" style={{ background: "#1a1a1a" }}>
          {items.map((it, i) => (
            <button key={it.id} data-idx={i} onClick={() => onIndexChange(i)}
              ...
              style={{
                width: 56, height: 56,
                outline: i === index ? "2px solid #2F6FED" : "2px solid transparent",
                outlineOffset: 2,
...
const BG_OPTIONS = [
  { id: "default",   label: "Padrão",      bg: "#f0ece8", style: { backgroundImage: "radial-gradient(circle, rgba(47,111,237,0.06) 1px, transparent 1px)", backgroundSize: "22px 22px" } },
  ...
  { id: "dots",      label: "Pontos",      bg: "#f5f0ff", style: { backgroundImage: "radial-gradient(circle, rgba(47,111,237,0.15) 1.5px, transparent 1.5px)", backgroundSize: "18px 18px" } },
  { id: "lines",     label: "Linhas",      bg: "#f0f4ff", style: { backgroundImage: "repeating-linear-gradient(0deg, transparent, transparent 22px, rgba(47,111,237,0.08) 22px, rgba(47,111,237,0.08) 23px)" } },
  { id: "grid",      label: "Grelha",      bg: "#f8f8f8", style: { backgroundImage: "linear-gradient(rgba(47,111,237,0.07) 1px,transparent 1px),linear-gradient(90deg,rgba(47,111,237,0.07) 1px,transparent 1px)", backgroundSize: "20px 20px" } },
    ...
    <div className="relative flex items-center gap-1 flex-wrap mt-0.5">
      ...
      {Object.entries(reactions ?? {}).filter(([,v])=>v>0).map(([emoji, count]) => (
        <button key={emoji} onClick={() => onReact(emoji)}
          ...
          style={{
            background: myReaction===emoji ? "#2F6FED20" : "var(--s3)",
            border: myReaction===emoji ? "1px solid #2F6FED50" : "1px solid var(--border-subtle)",
          }}>
    ...
    return (
      <div className={`flex ${isMe ? "justify-end" : "justify-start"} px-1`}>
        <div className="flex items-center gap-1.5 px-3 py-2 rounded-2xl text-xs italic"
          style={{ background: isMe ? "#2F6FED22" : "var(--s2)", color: "var(--text-muted)" }}>
  ...
  const menuActions = (
    <>
      ...
      <button onClick={() => { onReply(); closeMenu(); }}
        ...
        <Reply className="h-4 w-4" style={{ color: "#2F6FED" }} />
      ...
      {isMe && m.type === "text" && (
        <button onClick={() => { onEdit(); closeMenu(); }}
          ...
          onMouseOver={e => e.currentTarget.style.background = "var(--s2)"}
          onMouseOut={e => e.currentTarget.style.background = "transparent"}>
          <Pencil className="h-4 w-4" style={{ color: "#2F6FED" }} />
          ...
          {replied && (
            <div className="px-3 pt-2 pb-0">
              <div className="px-2 py-1 rounded-lg text-xs"
                style={{ background: isMe ? "rgba(255,255,255,0.2)" : "var(--s3)", borderLeft: "3px solid " + (isMe ? "white" : "#2F6FED") }}>
                <p className="font-bold mb-0.5" style={{ color: isMe ? "rgba(255,255,255,0.9)" : "#2F6FED" }}>
          ...
          <div className={isStickerBubble ? "" : "px-3 py-2"}>
            ...
            {m.viewOnce && !m.viewOnceOpened && (m.type === "image" || m.type === "video") && (
              <button onClick={onOpenViewOnce}
                ...
                style={{ background: isMe ? "rgba(255,255,255,0.15)" : "#2F6FED15", color: isMe ? "white" : "#2F6FED" }}>
            ...
            {m.type === "file" && m.mediaUrl && (
              <button type="button" onClick={() => downloadChatFile(m.mediaUrl!, m.text)}
                ...
                <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0"
                  style={{ background: isMe ? "rgba(255,255,255,0.2)" : "#2F6FED" }}>
            ...
            >
              <div
                ...
                style={{
                  background: opt.bg,
                  ...opt.style,
                  outline: current === opt.id ? "3px solid #2F6FED" : "2px solid transparent",
                  outlineOffset: 2,
                  boxShadow: current === opt.id ? "0 0 0 4px #2F6FED22" : undefined,
                }}
              />
              <span className="text-[10px] font-semibold text-center leading-tight"
                style={{ color: current === opt.id ? "#2F6FED" : "var(--text-muted,#888)" }}>
                {opt.label}
              </span>
            </button>
          ))}
        ...
        {filtered.map(c => {
          ...
          return (
            <button key={c.id} onClick={() => toggle(c.id)}
              ...
              style={{ background: isSelected ? "#2F6FED15" : "transparent" }}>
              ...
              <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0"
                style={{
                  background: isSelected ? "#2F6FED" : "transparent",
...
function ChatPanel({ myId, contact, onBack, contacts }: {
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: CHAT_BACKGROUNDS[bgId] ?? "var(--s1)" }}>
      ...
      {/* ── HEADER — limpo, neutro, sem gradiente ── */}
      <div className="flex items-center gap-3 px-3 py-2.5 shrink-0 z-10"
        style={{ background: "var(--s0,#fff)", borderBottom: "1px solid var(--border-subtle,#f0f0f0)" }}>
        ...
        <div className="flex-1 min-w-0 cursor-pointer">
          <p className="text-sm font-bold truncate leading-tight flex items-center gap-1" style={{ color: "var(--text-primary)" }}>
          ...
          <p className="text-[11px] flex items-center gap-1" style={{ color: "var(--text-muted)" }}>
            {(contact as Contact).isOfficial
              ...
              : presenceLabel
                ? (<>
                    {contactTyping && <span className="inline-block w-1.5 h-1.5 rounded-full bg-[#22c55e] animate-pulse" />}
        ...
        <div
          ...
          style={{
            ...
            color: canEncrypt ? "#22c55e" : "var(--text-muted)",
          }}
        >
            ...
            <div className="absolute right-0 top-11 z-50 w-64 rounded-2xl shadow-xl border overflow-hidden"
              ...
              <button onClick={e => { e.stopPropagation(); toggleReadReceipts(); }}
                ...
                <div className="flex items-center gap-3">
                  <CheckCheck className="h-4 w-4" style={{ color: readReceipts ? "#2F6FED" : "var(--text-muted)" }} />
                  <div>
                ...
                <div className={`w-10 h-5 rounded-full transition-colors relative shrink-0 ${readReceipts ? "bg-[#2F6FED]" : "bg-neutral-400"}`}>
              ...
              <button onClick={e => { e.stopPropagation(); toggleLastSeen(); }}
                ...
                <div className="flex items-center gap-3">
                  <Eye className="h-4 w-4" style={{ color: showLastSeen ? "#2F6FED" : "var(--text-muted)" }} />
                  <div>
                ...
                <div className={`w-10 h-5 rounded-full transition-colors relative shrink-0 ${showLastSeen ? "bg-[#2F6FED]" : "bg-neutral-400"}`}>
              ...
              <button onClick={e => { e.stopPropagation(); toggleMute(); }}
                ...
                style={{ borderColor:"var(--border-default)" }}>
                <div className="flex items-center gap-3">
                  <Bell className="h-4 w-4" style={{ color: mutedConv ? "var(--text-muted)" : "#2F6FED" }} />
                  <div>
                ...
                <div className={`w-10 h-5 rounded-full transition-colors relative shrink-0 ${mutedConv ? "bg-neutral-400" : "bg-[#2F6FED]"}`}>
          ...
          if (merged.deletedForAll) {
            return (
              <div key={merged.id} className={`flex ${isMe ? "justify-end" : "justify-start"} px-1`}>
                <div className="flex items-center gap-1.5 px-3 py-2 rounded-2xl text-xs italic"
                  style={{ background: isMe ? "#2F6FED15" : "var(--s2)", color: "var(--text-muted)" }}>
                    ...
                    {contact.avatar_url
                      ...
                      : (
                        <div className="w-full h-full flex items-center justify-center text-[8px] font-bold text-white"
                          style={{ background: contact.color || "#2F6FED" }}>
      ...
      {showScrollBtn && (
        <button onClick={() => { atBottom.current = true; setShowScrollBtn(false); bottomRef.current?.scrollIntoView({ behavior:"smooth" }); }}
          ...
          style={{ background:"#2F6FED", color:"white", position:"sticky" }}>
      ...
      {editingMsgId && (
        <div className="flex items-center gap-2 px-3 py-2 shrink-0 border-t"
          style={{ background:"var(--s2)", borderColor:"#2F6FED44" }}>
          <Pencil className="h-4 w-4 shrink-0" style={{ color:"#2F6FED" }} />
          ...
          <button onClick={confirmEdit}
            ...
            style={{ background:"#2F6FED" }}>Guardar</button>
      ...
      {showAttach && !recording && (
        <div className="px-3 py-4 grid grid-cols-6 gap-2 shrink-0 border-t"
          ...
          {[
            { icon: <ImageIcon className="h-5 w-5 text-white"/>, label:"Galeria", color:"#2F6FED", action:() => imgInputRef.current?.click() },
      ...
      {/* Nota: a UI de gravação deixou de ser uma barra separada por cima do
          campo — agora vive DENTRO da própria caixa de escrever, estilo
          Facebook Messenger (ver "INPUT BAR" mais abaixo). */}
        ...
        <div className="shrink-0 border-t px-3 py-3"
          ...
          <div className="flex items-center gap-3">
                  ...
                  {waveform.map((h, i) => {
                    ...
                    return (
                      <div key={i} className="flex-1 rounded-full transition-colors duration-75"
                        style={{
                          height:`${h}%`,
                          background: !inTrim ? "rgba(255,255,255,0.12)" : played ? "#2F6FED" : "rgba(255,255,255,0.28)",
                        }} />
                    );
                  })}
                </div>
            ...
            <button onClick={sendAudioPreview}
              className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 transition active:scale-90"
              style={{ background:"#2F6FED", color:"white", boxShadow:"0 2px 8px #2F6FED55" }}>
              <Send className="h-5 w-5" style={{ marginLeft:1 }} />
            </button>
          </div>
          {/* Linha de corte — compacta, abaixo da bolha */}
          <div className="flex items-center gap-2 mt-2.5 px-1">
            <span className="text-[9px] shrink-0" style={{ color:"#2F6FED", minWidth:28, textAlign:"right" }}>
            ...
            <div className="flex-1 flex flex-col gap-1">
              <input type="range" min={0} max={trimEnd - 2} value={trimStart}
                ...
                className="w-full h-1 accent-[#2F6FED]" style={{ cursor:"pointer" }} />
              <input type="range" min={trimStart + 2} max={100} value={trimEnd}
                ...
                className="w-full h-1" style={{ cursor:"pointer", accentColor:"#2F6FED" }} />
            </div>
            <span className="text-[9px] shrink-0" style={{ color:"#2F6FED", minWidth:28 }}>
      ...
      <div className="flex items-end gap-2 px-3 py-2 shrink-0"
        ...
        {recording ? (
          <>
            {/* Cancelar gravação — substitui o botão de anexar enquanto grava */}
            <button onClick={cancelRecording}
              className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 transition active:scale-90 mb-0.5"
              style={{ background:"#EF444415", color:"#EF4444" }}>
              <Trash2 className="h-5 w-5" />
            </button>
            {/* Mesma caixa arredondada do campo de texto, agora com o
                indicador de gravação (ponto + waveform + timer) — estilo
                Facebook Messenger: a caixa "transforma-se", não aparece
                uma barra nova por cima. */}
            <div className="flex-1 flex items-center rounded-3xl px-4 py-2 gap-2 min-h-[44px]"
              ...
              <div className="flex gap-[2px] items-end flex-1 h-6 overflow-hidden">
        ...
        <button onClick={() => { setShowAttach(v=>!v); setShowEmoji(false); }}
          className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 transition active:scale-90 mb-0.5"
          style={{
            background: showAttach ? "#2F6FED" : "white",
            color: showAttach ? "white" : "#2F6FED",
        ...
        <div className="flex-1 flex items-end rounded-3xl px-4 py-2 gap-2 min-h-[44px] transition-shadow focus-within:shadow-[0_0_0_2px_rgba(47,111,237,0.25)]"
        ...
        {input.trim() && !recording
          ? (
            <button onClick={() => send()} disabled={sending}
              ...
              style={{ background:`#2F6FED`, color:"white", boxShadow:`0 4px 14px rgba(47,111,237,0.4)` }}>
            ...
            <button
              ...
              className="w-11 h-11 rounded-full flex items-center justify-center shrink-0 transition active:scale-90 mb-0.5"
              style={{ background: recording ? "#EF4444" : "#2F6FED", color:"white", boxShadow: recording ? "0 4px 14px #EF444455" : "0 4px 14px #2F6FED55" }}>
...
function ContactList({ contacts, loading, refreshing, search, setSearch, active, setActive, setShowAddContact, setShowRequests, pendingRequestCount, officialMessages, activeOfficialId, onSelectOfficial, activeSalaSlug, onSelectSala }: {
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: "var(--surface-0,#fff)" }}>
      {/* Header — limpo, neutro, sem gradientes; um único acento azul */}
      <div className="px-4 pt-6 pb-3 shrink-0" style={{ borderBottom: "1px solid var(--border-subtle,#f0f0f0)" }}>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold tracking-tight" style={{ color: "var(--text-primary,#111)" }}>Mensagens</h1>
          ...
          <div className="flex gap-1">
            ...
            <button onClick={() => setShowAddContact(true)}
              ...
              onMouseOver={e => e.currentTarget.style.background = "var(--s2)"}
              onMouseOut={e => e.currentTarget.style.background = "transparent"}>
              <UserPlus className="h-5 w-5" style={{ color: "var(--text-secondary)" }} />
            </button>
          </div>
        </div>
        ...
        {filtered.map((c: any) => (
          <button
            ...
            style={{
              ...
              background: active?.conversationId === c.conversationId ? "var(--s2)" : "transparent",
            }}
            onMouseOver={e => { if (active?.conversationId !== c.conversationId) e.currentTarget.style.background = "var(--s1)"; }}
            onMouseOut={e => { if (active?.conversationId !== c.conversationId) e.currentTarget.style.background = "transparent"; }}>
            {/* Barra de acento à esquerda quando activa — em vez de anel/gradiente */}
            {active?.conversationId === c.conversationId && (
              <span className="absolute left-0 top-0 bottom-0 w-[3px]" style={{ background: "#2F6FED" }} />
            )}
            ...
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-baseline gap-2">
                <p className={`text-sm truncate flex items-center gap-1 ${c.unread > 0 ? "font-bold" : "font-medium"}`} style={{ color: "var(--text-primary,#111)" }}>
                  <span className="truncate">{c.full_name || c.username}</span>
                  {c.isOfficial && <VerifiedBadge />}
                </p>
                <p className="text-[11px] shrink-0" style={{ color: c.unread > 0 ? "#2F6FED" : "var(--text-muted,#aaa)" }}>
                  {c.lastTime}
                </p>
              </div>
              <div className="flex items-center justify-between gap-2 mt-0.5">
                <p
                  className={`text-[13px] truncate flex-1 ${c.unread > 0 ? "font-semibold" : "font-normal"}`}
                  style={{ color: c.unread > 0 ? "var(--text-primary,#111)" : "var(--text-muted,#888)" }}
                >
                  {c.lastMsg || "@" + c.username}
                </p>
                {c.unread > 0 && (
                  <span className="min-w-[19px] h-[19px] px-1.5 rounded-full text-[11px] font-bold text-white flex items-center justify-center shrink-0"
                    style={{ background: "#2F6FED" }}>
    ...
    return (
      <PageWrapper>
        <div className="flex items-center justify-center h-screen">
          <Loader className="h-8 w-8 animate-spin" style={{ color: "#2F6FED" }} />
        </div>
      </PageWrapper>
    );
  }
