...
export function SalaPanel({ slug, onBack }: { slug: string; onBack: () => void }) {
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-full"><Loader2 className="w-6 h-6 animate-spin" style={{ color: P }} /></div>;
  }
  ...
  if (!sala) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-3">
        ...
        <button onClick={onBack} className="px-4 h-9 rounded-full text-white text-sm font-bold" style={{ background: P }}>Voltar a Mensagens</button>
      </div>
    );
  }
      ...
      <div className="flex flex-col h-full" style={{ background: "var(--s1)" }}>
        ...
        <div className="shrink-0 z-10 px-4 pt-4 pb-3" style={{ background: "var(--s1)", borderBottom: "1px solid var(--border-subtle)" }}>
          <div className="flex items-center gap-2 mb-3">
            <button onClick={onBack} className="p-1.5 rounded-full" style={{ background: "var(--s2)" }}>
