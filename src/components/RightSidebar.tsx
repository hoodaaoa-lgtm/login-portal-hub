...
export function RightSidebar() {
  ...
  return (
    <aside
      className="hidden xl:flex flex-col h-screen sticky top-0 overflow-y-auto py-4 space-y-4"
          ...
          <>
            {suggestions.map(u => (
              <div key={u.id} className="flex items-center gap-3 px-4 py-2.5">
                <button
                  onClick={() => navigate({ to: "/u/$username", params: { username: u.username } })}
                  className="flex items-center gap-3 flex-1 min-w-0 text-left"
                >
                  <div
                    className="h-9 w-9 rounded-full shrink-0 overflow-hidden flex items-center justify-center text-white text-sm font-bold"
