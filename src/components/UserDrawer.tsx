...
function Avatar({ user, size = 38 }: { user: { full_name?: string | null; username?: string | null; avatar_url?: string | null }; size?: number }) {
  ...
  return (
    <div className="rounded-full overflow-hidden shrink-0 flex items-center justify-center text-white font-bold"
      style={{ width: size, height: size, background: user.avatar_url ? "transparent" : "#2F6FED", fontSize: size * 0.42 }}>
      {user.avatar_url
  ...
  const MENU_SECTIONS = [
    ...
    },
    {
      ...
      items: [
        ...
        { icon: BookOpen, label: "Livros", color: "#2F6FED", action: () => { onClose(); navigate({ to: "/livros" as any }); } },
      ],
    },
    {
      title: "Definições",
  ...
  const emptyState = (label: string, Icon: any) => (
    <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
      <div className="w-14 h-14 rounded-full flex items-center justify-center mb-3"
        ...
        <Icon className="h-6 w-6" style={{ color: "#2F6FED" }} />
      </div>
      ...
      return (
        <>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-2">
            ...
            {ratings === null
              ? <div className="py-8 flex justify-center"><div className="h-6 w-6 rounded-full border-2 animate-spin" style={{ borderColor: "#2F6FED", borderTopColor: "transparent" }} /></div>
              : ratings.length === 0
              ...
              {!loading && profile ? (
                <button onClick={() => { onClose(); navigate({ to: "/perfil" }); }}
                  ...
                  <div className="w-14 h-14 rounded-full overflow-hidden shrink-0 flex items-center justify-center text-white font-bold text-xl"
                    style={{ background: profile.avatar_url ? "transparent" : "#2F6FED" }}>
                    {profile.avatar_url
              ...
              <button onClick={toggle}
                ...
                <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0"
                  ...
                  {theme === "dark"
                    ...
                    : <Moon className="h-5 w-5" style={{ color: "#2F6FED" }} strokeWidth={1.8} />}
                </div>
