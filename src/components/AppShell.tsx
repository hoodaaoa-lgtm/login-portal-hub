...
import {
  Home, Compass, User, Tv, Menu,
  Moon, Sun, Bell, MoreHorizontal, ArrowLeft,
} from "lucide-react";
import { MessagesRound } from "@/components/icons/MessagesRound";
...
const NAV_ITEMS = [
  ...
  { to: "/mensagens",  label: t("nav.messages"),    Icon: MessagesRound, search: undefined as Record<string, string> | undefined },
...
function badgeCountFor(to: string, unreadMessages: number, label?: string, unreadNotifications?: number): number {
  if (label === "Notificações") return unreadNotifications || 0;
            ...
            return (
              <button key={label}
                ...
                <span className="ml-auto flex items-center gap-2">
                  <NavCountBadge count={badgeCountFor(to, unreadMessages, label, unreadNotifications)} />
                </span>
              </button>
            );
          }
            ...
            <Link key={label} to={to} search={search}
              ...
              {isPerfil ? (
                <div className="shrink-0 h-7 w-7 rounded-full overflow-hidden flex items-center justify-center text-[11px] font-extrabold text-white"
                  style={{ background: avatarUrl ? "transparent" : "#2F6FED" }}>
                  {avatarUrl
              ...
              <li key={label}>
                {isNotif ? (
                  ...
                  <Link to={to as string} search={search}
                    className="flex flex-col items-center justify-center gap-1 h-full w-full min-w-0 px-0.5 transition-all duration-150 active:scale-90"
                    style={{ color: active ? "#2F6FED" : "var(--text-muted)" }}>
                    <div className="relative flex items-center justify-center"
                      style={{
                        width: 38, height: 28, borderRadius: 12,
