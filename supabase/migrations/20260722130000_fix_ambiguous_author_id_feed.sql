-- ───────────────────────────────────────────────────────────────────────
-- Corrige erro 42702 ("column reference author_id is ambiguous") na
-- get_personalized_feed_v3: como a função devolve uma coluna chamada
-- author_id (via RETURNS TABLE), qualquer referência SEM PREFIXO a
-- "author_id" dentro do corpo da função fica ambígua para o Postgres —
-- pode ser essa coluna de saída OU a coluna da tabela/CTE. Isto só dava
-- erro real quando a query chegava a executar essa parte (o que passou a
-- acontecer depois de recriarmos a tabela "follows").
-- Correção: qualifica sempre "author_id" com o nome da CTE de origem.
-- ───────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  b1 AS (
    SELECT scored.id, scored.author_id, scored.top_category, score_interest AS rank_score, 'interesses'::TEXT AS bucket
    FROM scored WHERE score_interest > 0
    ORDER BY score_interest DESC LIMIT n1
  ),
  b2 AS (
    SELECT scored.id, scored.author_id, scored.top_category, score_followed AS rank_score, 'seguidos'::TEXT AS bucket
    FROM scored WHERE is_followed AND id NOT IN (SELECT b1.id FROM b1)
    ORDER BY score_followed DESC LIMIT n2
  ),
  b3 AS (
    SELECT scored.id, scored.author_id, scored.top_category, score_similaridade AS rank_score, 'similaridade'::TEXT AS bucket
    FROM scored WHERE id NOT IN (SELECT b1.id FROM b1 UNION SELECT b2.id FROM b2)
    ORDER BY score_similaridade DESC LIMIT n3
  ),
  b4 AS (
    SELECT scored.id, scored.author_id, scored.top_category, score_discovery AS rank_score, 'descoberta'::TEXT AS bucket
    FROM scored WHERE NOT is_followed AND id NOT IN (SELECT b1.id FROM b1 UNION SELECT b2.id FROM b2 UNION SELECT b3.id FROM b3)
    ORDER BY score_discovery DESC LIMIT n4
  ),
  b5 AS (
    SELECT scored.id, scored.author_id, scored.top_category, score_trending AS rank_score, 'tendencias'::TEXT AS bucket
    FROM scored WHERE id NOT IN (SELECT b1.id FROM b1 UNION SELECT b2.id FROM b2 UNION SELECT b3.id FROM b3 UNION SELECT b4.id FROM b4)
  ...
  filler AS (
    ...
    FROM scored s
    WHERE s.id NOT IN (SELECT combined.id FROM combined)
