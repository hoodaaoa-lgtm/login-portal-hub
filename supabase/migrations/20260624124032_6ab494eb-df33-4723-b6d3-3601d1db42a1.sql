...
CREATE POLICY "library_books read all" ON public.library_books FOR SELECT TO authenticated USING (true);
CREATE POLICY "library_books insert own" ON public.library_books FOR INSERT TO authenticated WITH CHECK (auth.uid() = author_id);
CREATE POLICY "library_books update own or counters" ON public.library_books FOR UPDATE TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "library_books delete own" ON public.library_books FOR DELETE TO authenticated USING (auth.uid() = author_id);
