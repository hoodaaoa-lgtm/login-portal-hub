-- ============================================================================
-- CORREÇÃO CRÍTICA — fase "Em Análise"/"Em Teste" não era respeitada
--
-- Bug: get_personalized_feed_v3 (feed global/página inicial/sugestões) só
-- excluía distribution_state = 'distribuicao_reduzida'. Uma publicação
-- recém-criada (default 'em_analise') era imediatamente entregue a TODA a
-- plataforma — a fase de análise da IA não bloqueava nada na prática.
--
-- Correção: candidates (e a repescagem) passam a exigir distribution_state
-- num destes 4 estados liberados para distribuição pública:
--   distribuicao_normal, em_crescimento, tendencia, viral
-- Ficam de fora do feed global: em_analise, em_teste, distribuicao_reduzida.
--
-- Nada muda em quem pode ver a publicação diretamente (perfil do autor,
-- link direto, seguidores no perfil) — isso é controlado por RLS/queries
-- próprias do perfil, que não usam esta função. Esta função controla
-- apenas o QUE ENTRA no feed público/algorítmico.
-- Resto da função (pesos, buckets, diversificar_feed) fica intacto —
-- só as duas linhas de filtro por distribution_state mudaram.
...
BEGIN
  ...
  candidates AS (
    ...
    WHERE p.created_at < p_cursor
      ...
      -- CORREÇÃO CRÍTICA: só entram no feed global publicações já
      -- liberadas pela IA. em_analise/em_teste/distribuicao_reduzida
      -- NUNCA aparecem aqui (antes só excluía distribuicao_reduzida).
  ...
  resurface AS (
    ...
    WHERE p.created_at < p_cursor
      AND p.author_id IS DISTINCT FROM p_user_id
      -- mesma regra na repescagem: o estado pode ter mudado (ex.: passou a
      -- moderação/reduzida) desde que foi visto pela primeira vez
      AND p.distribution_state IN ('distribuicao_normal', 'em_crescimento', 'tendencia', 'viral')
