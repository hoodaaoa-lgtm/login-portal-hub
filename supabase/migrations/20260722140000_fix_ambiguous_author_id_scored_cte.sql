-- ───────────────────────────────────────────────────────────────────────
-- Corrige (outra vez) o erro 42702 "column reference author_id is
-- ambiguous" na get_personalized_feed_v3.
--
-- A migração anterior (20260722130000_fix_ambiguous_author_id_feed.sql)
-- qualificou author_id/top_category em quase todos os CTEs, mas o
-- próprio CTE "scored" continuou a listar as colunas sem prefixo:
--   SELECT id, author_id, created_at, is_followed, top_category, ...
-- Como a função devolve colunas de saída chamadas author_id e
-- top_category (RETURNS TABLE), qualquer referência sem prefixo dentro
-- do corpo da função é ambígua para o Postgres. Esta migração qualifica
-- também esse SELECT com o alias da CTE de origem ("candidates").
