-- ───────────────────────────────────────────────────────────────────────
-- Repõe o gate de distribution_state na get_personalized_feed_v3.
--
-- A migração 20260714140000_remove_redes_feature.sql, ao reverter a
-- funcionalidade "Redes", recriou esta função a partir de uma versão
-- anterior ao fix de 20260713150000_fix_distribution_gate.sql — e com
-- isso perdeu, sem querer, o filtro que impede publicações em
-- 'em_analise' (ou 'em_teste'/'distribuicao_reduzida') de aparecerem no
-- feed público antes de serem liberadas pela IA. Todas as migrações
-- seguintes (feed_so_redes, fix_ambiguous_author_id_*) foram criadas em
-- cima dessa versão sem o filtro, por isso o problema persistiu até agora.
--
-- Correção: candidates e a repescagem voltam a exigir
--   distribution_state IN ('distribuicao_normal', 'em_crescimento', 'tendencia', 'viral')
-- Nenhuma outra lógica foi alterada. Idempotente.
...
BEGIN
  ...
  b2 AS (
    ...
    FROM scored WHERE scored.is_followed AND scored.id NOT IN (SELECT b1.id FROM b1)
  ...
  b3 AS (
    ...
    FROM scored WHERE scored.id NOT IN (SELECT b1.id FROM b1 UNION SELECT b2.id FROM b2)
  ...
  b4 AS (
    ...
    FROM scored WHERE NOT scored.is_followed AND scored.id NOT IN (SELECT b1.id FROM b1 UNION SELECT b2.id FROM b2 UNION SELECT b3.id FROM b3)
  ...
  b5 AS (
    ...
    FROM scored WHERE scored.id NOT IN (SELECT b1.id FROM b1 UNION SELECT b2.id FROM b2 UNION SELECT b3.id FROM b3 UNION SELECT b4.id FROM b4)
